...
var StringRepository = (function () {
  function StringRepository(pool, table, column) {
    this.pool = pool;
    this.table = table;
    this.column = column;
  ...
  StringRepository.prototype.load = function (key, max) {
    var _this = this;
  ...
  StringRepository.prototype.save = function (values) {
...
var StringService = (function () {
  function StringService(table, field, query, exec) {
    this.table = table;
    this.field = field;
    this.query = query;
    this.exec = exec;
  ...
  StringService.prototype.save = function (values) {
    if (!values || values.length === 0) {
      ...
      var s = buildStatements(this.table, this.field, values);
      return this.exec(s);
    }
  };
...
function buildStatements(table, field, values) {
  var s = [];
  for (var _i = 0, values_1 = values; _i < values_1.length; _i++) {
    var v = values_1[_i];
    if (v && v.length > 0) {
      var query_1 = "insert into " + table + "(" + field + ") values ($1) on conflict(" + field + ") do nothing";
      s.push({ query: query_1, params: [v] });
    }
  }
  return s;
}
