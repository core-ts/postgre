...
var UrlQuery = (function () {
  function UrlQuery(queryF, table, url, id) {
    this.queryF = queryF;
    this.table = table;
  ...
  UrlQuery.prototype.query = function (ids) {
    ...
    var sql = "select " + this.id + " as id, " + this.url + " as url from " + this.table + " where " + this.id + " in (" + pv.join(',') + ") and " + this.url + " is not null order by " + this.id;
    return this.queryF(sql, ps);
  };
  return UrlQuery;
}());
exports.UrlQuery = UrlQuery;
function useUrlQuery(queryF, table, url, id) {
  var q = new UrlQuery(queryF, table, url, id);
  return q.query;
}
...
var ReactionService = (function () {
  ...
  ReactionService.prototype.react = function (id, author, reaction) {
    var _this = this;
    var query = "select reaction from " + this.userreactionTable + " where " + this.id + " = $1 and " + this.author + " = $2";
    return this.db.query(query, [id, author]).then(function (r) {
      if (r.length <= 0) {
        var obj = {
          l1: '0',
          l2: '0',
...
exports.ReactService = ReactionService;
exports.ReactRepository = ReactionService;
