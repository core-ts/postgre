...
var ArrayRepository = (function () {
  function ArrayRepository(query, exec, table, field, id) {
    this.query = query;
    this.exec = exec;
    this.table = table;
    this.field = field;
  ...
  ArrayRepository.prototype.load = function (id) {
    return this.query("select " + this.id + " as id, " + this.field + " as items from " + this.table + " where " + this.id + " = $1", [id]).then(function (objs) {
      if (objs && objs.length > 0) {
